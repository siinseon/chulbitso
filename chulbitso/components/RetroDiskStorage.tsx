 ...
 */
export default function RetroDiskStorage({ books, className = "" }: RetroDiskStorageProps) {
  ...
  return (
    <div
      className={`rounded-2xl p-5 sm:p-6 border border-secondary flex flex-col min-h-[420px] ${className}`}
      style={{
    ...
    >
      ...
      <div className="flex justify-center flex-1 min-h-[300px]">
        <BookshelfWithSpines shelvesData={shelvesData} onSpineClick={openReview} />
      </div>
...
/** 책등 모양 플로피 디스크 1개 — 세로로 길고 두껍게, 세로 글씨 */
  ...
  return (
    <button
      ...
      style={{
        ...
        boxShadow: "inset 1px 0 0 rgba(255,255,255,0.15), 0 1px 2px rgba(0,0,0,0.12)",
      }}
      onClick={onClick}
      title={title}
    >
      ...
      <div
        ...
        style={{
          background: `linear-gradient(180deg, ${color} 0%, ${darken(color, 16)} 100%)`,
          boxShadow: "inset 0 1px 0 rgba(255,255,255,0.08)",
        }}
      >
        <span
          className="block text-[10px] font-medium text-white/95 py-0.5 overflow-hidden"
          style={{
...
function BookshelfWithSpines({
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: 440, height: 220, aspectRatio: "512/256" }}>
      <BookshelfFrameSvg className="absolute inset-0 w-full h-full object-contain opacity-95" />
      {/* 선반 3줄 — 각 행의 bottom을 선(y=82,164,251)에 정확히 맞춤 */}
      {shelvesData.map((items, shelfIndex) => (
        <div
          key={shelfIndex}
          className="absolute left-[6.64%] right-[6.64%] flex flex-wrap items-end gap-0.5 content-end"
          style={{
        ...
        >
          {items.length === 0 ? (
            <span className="text-[10px] text-[#8a7a6a] font-serif">비어 있음</span>
          ) : (
