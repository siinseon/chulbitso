...
type OpeningStep = "zoom" | "shake" | "crack" | "reveal";
...
export default function DestinyGashaponModal({
  ...
  const [openingStep, setOpeningStep] = useState<OpeningStep>("zoom");
  ...
  useEffect(() => {
    ...
    const t4 = setTimeout(() => setPhase("result"), delays[0] + delays[1] + delays[2] + delays[3]);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
      clearTimeout(t3);
      ...
      <AnimatePresence mode="wait">
        {phase !== "result" ? (
                ...
                >
                  ...
                  <AnimatePresence>
                    {phase === "opening" && (openingStep === "crack" || openingStep === "reveal") && (
                      <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        transition={{
                          duration: openingStep === "crack" ? 0.2 : 0.35,
                          ease: "easeOut",
                        }}
                        style={{
                          background: "radial-gradient(circle, rgba(255,255,255,0.95) 0%, rgba(255,230,180,0.8) 40%, transparent 70%)",
                        }}
                      />
                    )}
                  </AnimatePresence>
                  ...
                  <motion.div
                    ...
                    animate={
                      phase === "dropping"
                        ? { scale: 1, opacity: 1 }
                        : phase === "opening" && openingStep === "zoom"
                          ? { scale: 1.2 }
                          : phase === "opening" && openingStep === "shake"
                            ? { scale: 1.2, x: [0, -6, 6, -5, 5, -4, 4, -2, 2, 0], y: [0, 2, -2, 1, -1, 0] }
                            : phase === "opening"
                              ? { scale: 1.2, x: 0, y: 0 }
                              : { scale: 1 }
                    }
                  ...
                  >
                    ...
                    <motion.div
                      ...
                      initial={{ scale: 0, opacity: 0 }}
                      animate={
                        phase === "opening" && openingStep === "reveal"
                          ? { scale: 1, opacity: 1 }
                          : { scale: 0, opacity: 0 }
                      }
                      transition={{
          ...
          >
            {selectedBook ? (
              <motion.div
                className="rounded-2xl p-6 bg-white/95 shadow-xl"
                initial={{ scale: 0.9 }}
                animate={{ scale: 1 }}
                transition={{ type: "spring", stiffness: 300, damping: 20 }}
              >
                <p className="text-center text-amber-800/90 text-base font-serif font-bold mb-3">오늘 운명의 책은?</p>
                <div className="flex justify-center mb-4">
                  <div className="w-24 aspect-[2/3] rounded-lg shadow-lg overflow-hidden">
                    {isValidCoverUrl(selectedBook.cover) ? (
                      <Image
                        src={selectedBook.cover!}
                        alt={selectedBook.title}
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button
                    type="button"
                    onClick={onClose}
                    className="w-full py-2.5 text-sm font-bold text-text-main hover:text-primary"
                  >
                    닫기
                  </button>
                </div>
              ...
              <motion.div
                className="rounded-2xl p-6 bg-white/95 shadow-xl text-center"
                initial={{ scale: 0.9 }}
                animate={{ scale: 1 }}
              >
