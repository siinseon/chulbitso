...
function StampBlock({
  ...
  return (
    <div className="stamp-container flex-shrink-0 flex flex-col items-center group relative pb-4 sm:pb-6 md:pb-8">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 100 120"
        className="w-16 sm:w-20 md:w-24 h-auto mx-auto transition-transform duration-150 ease-out cursor-pointer group-hover:translate-y-2 group-active:translate-y-2"
      >
        <defs>
          {/* 손잡이 — 갈색에 가까운 붉은 플라스틱 */}
      ...
      <div
        className="absolute left-1/2 -translate-x-1/2 top-full -mt-3 sm:-mt-5 flex items-center justify-center rounded-full opacity-0 transition-opacity duration-150 delay-75 group-hover:opacity-100 group-active:opacity-100 min-w-[1.5rem] min-h-[1.5rem] sm:min-w-[2.25rem] sm:min-h-[2.25rem]"
        style={{
...
export default function TripleSwingsSection({ topAuthors }: TripleSwingsSectionProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative rounded-xl overflow-visible flex items-end justify-center pt-6 sm:pt-8 pb-0 px-2 sm:px-4 min-h-[160px] sm:min-h-[200px] gap-2 sm:gap-3 md:gap-5"
        style={{
          background: "linear-gradient(180deg, #F2E6D0 0%, #E8DCC8 100%)",
        }}
      >
