...
const SEAT_COLORS = ["#8C9E83", "#6A8B9A", "#C98C6E"] as const; // 세이지 그린, 빈티지 블루, 녹슨 오렌지

/* 배치: 좌=2위, 중앙=1위, 우=3위 */
const ORDER = [1, 0, 2] as const; // topAuthors[index] → left, center, right
...
function SingleSwing({ author, animClass, seatColor, onPush, isPushing }: SwingProps) {
...
export default function TripleSwingsSection({ topAuthors }: TripleSwingsSectionProps) {
  ...
  const items: (AuthorStat | null)[] = [];
  for (let i = 0; i < 3; i++) {
    const idx = ORDER[i];
    items.push(topAuthors[idx] ?? null);
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-[340px] mx-auto overflow-visible" style={{ height: 260 }}>
        ...
        {/* 왼쪽 세로 기둥 (가로 봉 끝에 붙음, 그네보다 약간 더 김) */}
        <div
