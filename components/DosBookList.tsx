...
export default function DosBookList({ books, onBookClick, appendDummyForTest }: DosBookListProps) {
  ...
  const listKey = `${statusFilter}-${genreFilter}`;
                ...
                <div className="flex gap-2 min-w-max">
                  {STATUS_MENUS.map(({ id, label }) => (
                    <button
                      ...
                      className="px-2 py-1.5 text-[12px] font-bold whitespace-nowrap border border-[rgba(0,255,0,0.3)]"
                      style={{ borderRadius: 0, ...(statusFilter === id ? { background: TITLE_COLOR, color: "#000", borderColor: TITLE_COLOR } : { background: "#000", color: TITLE_COLOR }) }}
                    >
                      [{label}]
                    </button>
                  ))}
                ...
                <div className="flex gap-2 min-w-max">
                  {GENRE_MENUS.map(({ id, label }) => (
                    <button
                      ...
                      className="px-2 py-1.5 text-[12px] font-bold whitespace-nowrap border border-[rgba(0,255,0,0.3)]"
                      style={{ borderRadius: 0, ...(genreFilter === id ? { background: TITLE_COLOR, color: "#000", borderColor: TITLE_COLOR } : { background: "#000", color: TITLE_COLOR }) }}
                    >
                      [{label}]
                    </button>
                  ))}
            ...
            <div className="flex-1 overflow-auto p-4 relative">
              ...
              <AnimatePresence mode="wait">
                <motion.pre
                  ...
                  style={{ textShadow: `0 0 8px ${TITLE_COLOR}, 0 0 16px ${TITLE_COLOR}80` }}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                ...
                >
                  ...
                  {lines.map((line, i) => (
                    <motion.span
                      key={i}
                      className={onBookClick ? "cursor-pointer hover:opacity-90" : ""}
                      onClick={() => onBookClick?.(displayBooks[i])}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      transition={{ duration: 0.02, delay: i * 0.02 }}
                    >
                      {line}
                      {"\n"}
                    </motion.span>
