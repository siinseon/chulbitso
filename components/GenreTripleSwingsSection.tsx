...
/* 순서: 좌=2위, 중=1위, 우=3위 */
const RANK_LABEL = ["1등", "2등", "3등"] as const;
...
function SingleSwing({ genre, rank, animClass, seatColor, onPush, isPushing }: SwingProps) {
  const [showTooltip, setShowTooltip] = useState(false);
  const displayName = genre?.name ?? "—";
  const count = genre?.count ?? 0;
  const rankLabel = RANK_LABEL[rank - 1];
  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      {showTooltip && genre && (
        ...
        >
          {rankLabel} {displayName} (총 {count}권)
          <div
      ...
      >
        ...
        <div
          className="w-full min-h-[32px] -mt-px px-2 py-2 text-center flex flex-col justify-center rounded-sm"
          style={{
        ...
        >
          <p
            className="text-[11px] font-extrabold text-[#1a1614] leading-tight truncate w-full drop-shadow-sm"
            style={{ letterSpacing: "0.03em", textShadow: "0 0.5px 0 rgba(255,255,255,0.4)" }}
            title={`${rankLabel} ${displayName}`}
          >
            {rankLabel} {displayName}
          </p>
        </div>
      </div>
    </div>
